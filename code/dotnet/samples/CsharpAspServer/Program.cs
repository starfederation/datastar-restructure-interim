...
public static class Program
{
    public static void Main(string[] args)
    {
        ...
        app.MapGet("/language/{lang:required}", (string lang, IServerSentEventGenerator sse) => sse.RenderFragment($"""<span id="language">{lang}</span>"""));
        ...
        app.MapGet("/target", async (IServerSentEventGenerator sse) =>
        {
            ...
            await sse.RenderFragment($"""<div id='target'><b>{today}</b><button data-on-click="$$get('/removeTarget')">Remove</button></div>""");
        });
        app.MapGet("/feed", async (IServerSentEventGenerator sse, CancellationToken ct) =>
        {
            while (!ct.IsCancellationRequested)
            {
                long rand = Random.Shared.NextInt64(1000000000000000000, 5999999999999999999);
                await sse.RenderFragment($"<span id='feed'>{rand}</span>");
